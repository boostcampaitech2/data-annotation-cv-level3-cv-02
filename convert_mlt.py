...
# LANGUAGE_MAP에 포함되지 않는 것들은 others로 처리
...
class MLT17Dataset(Dataset):
    ...
    def parse_label_file(self, label_path):
        # points가 잘못된 경우 rearrange
        def rearrange_points(points):
            start_idx = np.argmin([np.linalg.norm(p, ord=1) for p in points])
            if start_idx != 0:
                # start_idx의 points 합이 가장 작지 않으면 가장 작은 값을 start point가 되도록 roll
...
def main():
    dst_image_dir = osp.join(DST_DATASET_DIR, 'images') # 이미지 복사해서 저장
    # dst_image_dir = None # 이걸로 하면 복사(저장) 안됨
